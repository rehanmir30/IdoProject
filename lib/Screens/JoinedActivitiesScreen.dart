...
class _JoinedActivitiesState extends State<JoinedActivities> {
  ...
  Future getAllActivities() async {
    ...
    {
      final datareference = await FirebaseDatabase.instance.reference().child("Activities").child(ActivtiyId[i]).once().then((value){
  ...
  getMembersLength() async{
    ...
    {
      final datareference = await FirebaseDatabase.instance.reference().child("Activities").child(MatchedActivtiyId[i]).once().then((value){
  ...
  getJoinedActivityDetail()async{
    ...
    for(int i=0; i<MatchedActivtiyId.length;i++)
    {
      ...
      await FirebaseDatabase.instance.reference().child("Activities").child(MatchedActivtiyId[i]).once().then((value){
        ...
        ActivityModel activityModel = ActivityModel(name: Name, manager: Manager, password: pass, id: MatchedActivtiyId[i]);
                            ...
                            child: GestureDetector(
                              onTap: (){
                                Navigator.push(
                                    ...
                                    MaterialPageRoute(
                                        builder: (context) =>
                                            ActivityScreen(ActivtiyDetail[index].id,widget.uid)));
                              },
                              child: Card(
